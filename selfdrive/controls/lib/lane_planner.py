...
class LanePlanner:
  ...
  def get_stock_path(self, CS, v_ego, path_t, path_xyz, vcurv):
    ...
    if len(vcurv) > 0:
      sLogger.Send(
        "vC" + "{:.2f}".format(vcurv[0]) +
        " LX" + "{:.1f}".format(self.lll_y[0]) +
        " RX" + "{:.1f}".format(self.rll_y[0]) +
        " LW" + "{:.1f}".format(self.lane_width) +
        " LP" + "{:.1f}".format(l_prob) +
        " RP" + "{:.1f}".format(r_prob) +
        " RS" + "{:.1f}".format(self.rll_std) +
        " LS" + "{:.1f}".format(self.lll_std))
    else:
      sLogger.Send(
        "vC--- LX" + "{:.1f}".format(self.lll_y[0]) +
        " RX" + "{:.1f}".format(self.rll_y[0]) +
        " LW" + "{:.1f}".format(self.lane_width) +
        " LP" + "{:.1f}".format(l_prob) +
        " RP" + "{:.1f}".format(r_prob) +
  ...
  def get_nlp_path(self, CS, v_ego, path_t, path_xyz, vcurv):
    ...
    if lane_trust > 0.025 and len(vcurv) == len(self.lll_y):
      ...
      # debug
      sLogger.Send(
        "Cf" + "{:.2f}".format(self.center_force) +
        " Mx" + "{:.2f}".format(final_ultimate_path_mix) +
        " vC" + "{:.2f}".format(vcurv[0]) +
        " LX" + "{:.1f}".format(self.lll_y[0]) +
        " RX" + "{:.1f}".format(self.rll_y[0]) +
        " LW" + "{:.1f}".format(self.lane_width) +
      ...
      if safe_idxs[0] and final_ultimate_path_mix > 0.0:
        path_xyz[:,1] = \
          final_ultimate_path_mix       * np.interp(path_t, self.ll_t[safe_idxs], self.ultimate_path[safe_idxs]) + \
          (1 - final_ultimate_path_mix) * path_xyz[:,1]
    else:
      self.center_force = 0.0
      self.tire_stiffness_multiplier = 1.0
