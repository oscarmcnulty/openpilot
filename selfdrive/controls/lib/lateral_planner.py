...
class LateralPlanner:
  ...
  def reset_mpc(self, x0=None):
    self.x0 = x0 if x0 is None else np.zeros(4)
  ...
  def update(self, sm):
    ...
    if len(md.lateralPlannerSolution.x) == TRAJECTORY_SIZE:
      self.model_plan = np.column_stack([
        md.lateralPlannerSolution.x,
        md.lateralPlannerSolution.y,
        md.lateralPlannerSolution.yaw,
        md.lateralPlannerSolution.yawRate])
