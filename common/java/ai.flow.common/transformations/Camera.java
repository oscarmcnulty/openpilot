...
public class Camera {
    ...
    private static float FocalX, FocalY, CenterX, CenterY;
     ...
     */
    public static void setCameraParams(float fx, float fy, float cx, float cy) {
        FocalX = fx;
        FocalY = fy;
        CenterX = cx;
        CenterY = cy;

        digital_zoom_apply = (FocalX + FocalY) * 0.5f / (utils.F2 ? Model.MEDMODEL_F2_FL : Model.MEDMODEL_FL);

        CameraIntrinsics = new float[]{
                FocalX, 0.0f, frameSize[0] * 0.5f + (CenterX - (frameSize[0] * 0.5f)) * digital_zoom_apply,
                0.0f, FocalY, frameSize[1] * 0.5f + (CenterY - (frameSize[1] * 0.5f)) * digital_zoom_apply,
                0.0f, 0.0f, 1.0f
        };
    ...
    public static float digital_zoom_apply = 1f;
    public static float[] CameraIntrinsics = null;
    ...
    public static INDArray cam_intrinsics = null;
